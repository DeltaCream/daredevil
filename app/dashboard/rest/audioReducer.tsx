...
type AudioState = {
    ...
    // singleAudioModeIdStack: Id[] | [];
    // items: Record<Id, AudioItem>;

    //ordered history of audio with id as key
    audioMeta: OrderedHistory<string, AudioItem>; 
};
...
const initialState: AudioState = {
    ...
    // singleAudioModeIdStack: [],
    // items: {},

    audioMeta: new OrderedHistory<string, AudioItem>(),
};
...
function reducer(state: AudioState, action: Action): AudioState {
    switch (action.type) {
        ...
        case "TOGGLE": {
            state.items[action.id].isPlaying =
                !state.items[action.id].isPlaying;
            return { ...state };
        }
        case "SET_VOLUME": {
            state.items[action.id].volume = action.value;
            return { ...state };
        }
        default:
            return state;
